...
typedef struct CPUClass {
    ...
    uint64_t (*get_pc)(CPUState *cpu);
    uint64_t (*get_reg)(CPUState *cpu, uint32_t regnum);
    uint64_t (*get_fp)(CPUState *cpu);
    uint32_t (*archname)(CPUState *cpu);

    uint64_t (*read_stack)(CPUState *cs, uint64_t addr, uint8_t* buf, uint64_t len);
    uint64_t (*maybe_adjust_addr)(CPUState *cs, uint64_t addr);
...
static inline uint32_t cpu_archname(CPUState *cpu)
{
    CPUClass *cc = CPU_GET_CLASS(cpu);
    return cc->archname(cpu);
}

static inline uint64_t cpu_get_pc(CPUState *cpu)
{
    CPUClass *cc = CPU_GET_CLASS(cpu);
...
static inline uint64_t cpu_get_reg(CPUState *cpu, uint32_t regnum)
{
    CPUClass *cc = CPU_GET_CLASS(cpu);

    return cc->get_reg(cpu, regnum);
}

static inline uint64_t cpu_get_fp(CPUState *cpu)
{
    CPUClass *cc = CPU_GET_CLASS(cpu);

    return cc->get_fp(cpu);
}


static inline uint64_t cpu_read_stack(CPUState *cpu, uint64_t addr, uint8_t* buf, uint64_t len)
{
    CPUClass *cc = CPU_GET_CLASS(cpu);
    return cc->read_stack(cpu, addr, buf, len);
}


static inline uint64_t cpu_maybe_adjust_addr(CPUState *cpu, uint64_t addr)
{
    CPUClass *cc = CPU_GET_CLASS(cpu);
    return cc->maybe_adjust_addr(cpu, addr);
}
