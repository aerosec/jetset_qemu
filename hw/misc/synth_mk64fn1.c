...
#include "hw/misc/synth_mk64fn1.h"
...
static void synth_realize(DeviceState *d, Error **errp)
{
    ...
    memory_region_init_io(
        &sio->io_block1, obj, &synth_io_ops_block1, sio, TYPE_SYNTH, 0x3FFFFFFF
    );
    sysbus_init_mmio(dev, &sio->io_block1);
    memory_region_add_subregion(sysbus, 0xA0000000, &sio->io_block1);



    memory_region_init_io(
        &sio->io_block2, obj, &synth_io_ops_block2, sio, TYPE_SYNTH, 0x100000
    );
    sysbus_init_mmio(dev, &sio->io_block2);
    memory_region_add_subregion(sysbus, 0x40000000, &sio->io_block2);
