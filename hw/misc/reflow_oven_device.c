...
#define TYPE_SYNTH "reflow_oven_device"
...
typedef struct {
    ...
    uint64_t io_read_trace_block1_0x40021004[22];

    uint64_t io_read_trace_block1_0x40022000[5];

    uint64_t io_read_trace_block1_0x40021018[84];

    uint64_t io_read_trace_block1_0x40010804[6];

    uint64_t io_read_trace_block1_0x40011000[4];

    uint64_t io_read_trace_block1_0x40010c00[7];

    uint64_t io_read_trace_block1_0x40010800[11];

    uint64_t io_read_trace_block1_0x40010004[3];

    uint64_t io_read_trace_block1_0x40010c04[3];

    uint64_t io_read_trace_block1_0x40021010[4];

    uint64_t io_read_trace_block1_0x4002101c[4];

    uint64_t io_read_trace_block1_0x4000440c[7];

    uint64_t io_read_trace_block1_0x40004410[4];

    uint64_t io_read_trace_block1_0x40004414[5];

    uint64_t io_read_trace_block1_0x40012408[19];

    uint64_t io_read_trace_block1_0x40012404[4];

    uint64_t io_read_trace_block1_0x4001242c[3];

    uint64_t io_read_trace_block1_0x40012434[3];

    uint64_t io_read_trace_block1_0x40012410[3];
...
static uint64_t synth_read_block1(void *opaque, hwaddr addr, unsigned size) {
    ...
    switch(addr) {
        ...
        case 0x40012408 - 0x40000000:
            if (sio->io_read_trace_block1_0x40012408[1] < sio->io_read_trace_block1_0x40012408[0]) {
                sio->io_read_trace_block1_0x40012408[1]++;
            } else {
                sio->io_read_trace_block1_0x40012408[1] -= 0;
            }
            res = sio->io_read_trace_block1_0x40012408[sio->io_read_trace_block1_0x40012408[1] + 1];
            break;


        case 0x40012404 - 0x40000000:
            if (sio->io_read_trace_block1_0x40012404[1] < sio->io_read_trace_block1_0x40012404[0]) {
                sio->io_read_trace_block1_0x40012404[1]++;
            } else {
                sio->io_read_trace_block1_0x40012404[1] -= 0;
            }
            res = sio->io_read_trace_block1_0x40012404[sio->io_read_trace_block1_0x40012404[1] + 1];
            break;


        case 0x4001242c - 0x40000000:
            if (sio->io_read_trace_block1_0x4001242c[1] < sio->io_read_trace_block1_0x4001242c[0]) {
                sio->io_read_trace_block1_0x4001242c[1]++;
            } else {
                sio->io_read_trace_block1_0x4001242c[1] -= 0;
            }
            res = sio->io_read_trace_block1_0x4001242c[sio->io_read_trace_block1_0x4001242c[1] + 1];
            break;


        case 0x40012434 - 0x40000000:
            if (sio->io_read_trace_block1_0x40012434[1] < sio->io_read_trace_block1_0x40012434[0]) {
                sio->io_read_trace_block1_0x40012434[1]++;
            } else {
                sio->io_read_trace_block1_0x40012434[1] -= 0;
            }
            res = sio->io_read_trace_block1_0x40012434[sio->io_read_trace_block1_0x40012434[1] + 1];
            break;


        case 0x40012410 - 0x40000000:
            if (sio->io_read_trace_block1_0x40012410[1] < sio->io_read_trace_block1_0x40012410[0]) {
                sio->io_read_trace_block1_0x40012410[1]++;
            } else {
                sio->io_read_trace_block1_0x40012410[1] -= 0;
            }
            res = sio->io_read_trace_block1_0x40012410[sio->io_read_trace_block1_0x40012410[1] + 1];
            break;
...
static void synth_reset(DeviceState *d)
{
    ...
    uint64_t trace_block1_0x40021004[] = {20, 0, 0x0, 0x0, 0x0, 0xc, 0x0, 0x0, 0x0, 0x8, 0x0, 0x0, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    for (int i = 0; i < 22; i++) {
    ...
    uint64_t trace_block1_0x40021018[] = {82, 0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    for (int i = 0; i < 84; i++) {
        sio->io_read_trace_block1_0x40021018[i] = trace_block1_0x40021018[i];
    }

    uint64_t trace_block1_0x40010804[] = {4, 0, 0x0, 0x0, 0x0, 0x0};
    for (int i = 0; i < 6; i++) {
        sio->io_read_trace_block1_0x40010804[i] = trace_block1_0x40010804[i];
    }

    uint64_t trace_block1_0x40011000[] = {2, 0, 0x0, 0x0};
    for (int i = 0; i < 4; i++) {
        sio->io_read_trace_block1_0x40011000[i] = trace_block1_0x40011000[i];
    }

    uint64_t trace_block1_0x40010c00[] = {5, 0, 0x0, 0x0, 0x0, 0x0, 0x0};
    for (int i = 0; i < 7; i++) {
        sio->io_read_trace_block1_0x40010c00[i] = trace_block1_0x40010c00[i];
    }

    uint64_t trace_block1_0x40010800[] = {9, 0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    for (int i = 0; i < 11; i++) {
        sio->io_read_trace_block1_0x40010800[i] = trace_block1_0x40010800[i];
    }

    uint64_t trace_block1_0x40010004[] = {1, 0, 0x0};
    for (int i = 0; i < 3; i++) {
        sio->io_read_trace_block1_0x40010004[i] = trace_block1_0x40010004[i];
    }

    uint64_t trace_block1_0x40010c04[] = {1, 0, 0x0};
    for (int i = 0; i < 3; i++) {
    ...
    uint64_t trace_block1_0x4000440c[] = {5, 0, 0x0, 0x0, 0x0, 0x0, 0x0};
    for (int i = 0; i < 7; i++) {
    ...
    uint64_t trace_block1_0x40012408[] = {17, 0, 0x1, 0x0, 0x0, 0x0, 0xe0000, 0x1, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x1, 0xe0000, 0x0, 0x100};
    for (int i = 0; i < 19; i++) {
        sio->io_read_trace_block1_0x40012408[i] = trace_block1_0x40012408[i];
    }

    uint64_t trace_block1_0x40012404[] = {2, 0, 0x0, 0x0};
    for (int i = 0; i < 4; i++) {
        sio->io_read_trace_block1_0x40012404[i] = trace_block1_0x40012404[i];
    }

    uint64_t trace_block1_0x4001242c[] = {1, 0, 0x0};
    for (int i = 0; i < 3; i++) {
        sio->io_read_trace_block1_0x4001242c[i] = trace_block1_0x4001242c[i];
    }

    uint64_t trace_block1_0x40012434[] = {1, 0, 0x0};
    for (int i = 0; i < 3; i++) {
        sio->io_read_trace_block1_0x40012434[i] = trace_block1_0x40012434[i];
    }

    uint64_t trace_block1_0x40012410[] = {1, 0, 0x0};
    for (int i = 0; i < 3; i++) {
        sio->io_read_trace_block1_0x40012410[i] = trace_block1_0x40012410[i];
    }
}
