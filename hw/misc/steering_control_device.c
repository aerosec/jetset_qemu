...
#define TYPE_SYNTH "steering_control_device"
...
typedef struct {
    ...
    uint64_t io_read_trace_peripherals_0x400e0e08[31];

    uint64_t io_read_trace_peripherals_0x400e1008[16];

    uint64_t io_read_trace_peripherals_0x400e1208[28];

    uint64_t io_read_trace_peripherals_0x400e1218[11];

    uint64_t io_read_trace_peripherals_0x400e1408[13];

    uint64_t io_read_trace_peripherals_0x400e1418[4];

    uint64_t io_read_trace_peripherals_0x400e0e18[9];

    uint64_t io_read_trace_peripherals_0x400e1018[4];
    ...
    uint64_t io_read_trace_peripherals_0x400e0618[8];

    uint64_t io_read_trace_peripherals_0x40084020[3];
...
static uint64_t synth_read_peripherals(void *opaque, hwaddr addr, unsigned size) {
    ...
    switch(addr) {
        ...
        case 0x40084020 - 0x40000000:
            if (sio->io_read_trace_peripherals_0x40084020[1] < sio->io_read_trace_peripherals_0x40084020[0]) {
                sio->io_read_trace_peripherals_0x40084020[1]++;
            } else {
                sio->io_read_trace_peripherals_0x40084020[1] -= 0;
            }
            res = sio->io_read_trace_peripherals_0x40084020[sio->io_read_trace_peripherals_0x40084020[1] + 1];
            break;
...
static void synth_reset(DeviceState *d)
{
    ...
    uint64_t trace_peripherals_0x400e0e08[] = {29, 0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x400, 0x0, 0x8000, 0x0, 0x0, 0x0, 0x10000, 0x0, 0x0, 0x0, 0x0, 0x10, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x2000000, 0x0, 0x0, 0x0, 0x100};
    for (int i = 0; i < 31; i++) {
    ...
    uint64_t trace_peripherals_0x400e1208[] = {26, 0, 0x0, 0x4000000, 0x0, 0x0, 0x0, 0x0, 0x200000, 0x0, 0x0, 0x4, 0x0, 0x0, 0x0, 0x0, 0x80, 0x0, 0x0, 0x80000, 0x0, 0x0, 0x0, 0x0, 0x4000, 0x0, 0x0, 0x0};
    for (int i = 0; i < 28; i++) {
        sio->io_read_trace_peripherals_0x400e1208[i] = trace_peripherals_0x400e1208[i];
    }

    uint64_t trace_peripherals_0x400e1218[] = {9, 0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4000, 0xffffeffe, 0xfffffffe, 0xfffffffe};
    for (int i = 0; i < 11; i++) {
    ...
    uint64_t trace_peripherals_0x400e0e18[] = {7, 0, 0x0, 0x8000, 0x0, 0x0, 0x0, 0x2000000, 0x0};
    for (int i = 0; i < 9; i++) {
        sio->io_read_trace_peripherals_0x400e0e18[i] = trace_peripherals_0x400e0e18[i];
    }

    uint64_t trace_peripherals_0x400e1018[] = {2, 0, 0x0, 0xfffffffe};
    for (int i = 0; i < 4; i++) {
    ...
    uint64_t trace_peripherals_0x400e0618[] = {6, 0, 0x100, 0x800, 0x2000, 0x2000, 0x0, 0x40000000};
    for (int i = 0; i < 8; i++) {
        sio->io_read_trace_peripherals_0x400e0618[i] = trace_peripherals_0x400e0618[i];
    }

    uint64_t trace_peripherals_0x40084020[] = {1, 0, 0x0};
    for (int i = 0; i < 3; i++) {
        sio->io_read_trace_peripherals_0x40084020[i] = trace_peripherals_0x40084020[i];
    }
}
