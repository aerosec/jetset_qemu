...
static uint64_t omap3_prm_read(void *opaque, hwaddr addr, unsigned size)
{
...
static uint64_t omap3_cm_read(void *opaque, hwaddr addr, unsigned size)
{
...
static inline uint32_t omap3_wdt_timer_read(struct omap3_wdt_s *timer)
{
    ...
    if (timer->active) {
        ...
        distance = muldiv64(distance, timer->rate, 1000);
...
static uint64_t omap3_wdt_read32(void *opaque, hwaddr addr,
                                 int wdt_index)
{
...
*/
static uint64_t omap3_mpu_wdt_read32(void *opaque, hwaddr addr, unsigned size)
{
    return omap3_wdt_read32(opaque, addr, OMAP3_MPU_WDT);
}

static void omap3_wdt_write32(void *opaque, hwaddr addr,
                              uint64_t value, int wdt_index)
{
    struct omap3_wdt_s *s = (struct omap3_wdt_s *) opaque;
...
static const MemoryRegionOps omap3_mpu_wdt_ops = {
    ...
    .read = omap3_mpu_wdt_read32,
    .write = omap3_mpu_wdt_write32,
    .endianness = DEVICE_NATIVE_ENDIAN,
};
...
struct omap3_scm_s {
	...
	uint8_t interface[48];     /*0x4800 2000*/
	uint8_t padconfs[576];     /*0x4800 2030*/
	uint32_t general[228];     /*0x4800 2270*/
	uint8_t mem_wkup[1024];    /*0x4800 2600*/
	uint8_t padconfs_wkup[96]; /*0x4800 2a00*/
	uint32_t general_wkup[8];  /*0x4800 2a60*/
};
...
static void omap3_scm_reset(struct omap3_scm_s *s)
{
	...
	padconfs = (uint32_t *)(s->padconfs_wkup);
...
*/
static uint64_t omap3_scm_read32(void *opaque, hwaddr addr, unsigned size)
{
    uint32_t v;
    v = omap3_scm_read8(opaque, addr);
    v |= omap3_scm_read8(opaque, addr + 1) << 8;
...
static void omap3_scm_write16(void *opaque, hwaddr addr,
                              uint32_t value)
{
    omap3_scm_write8(opaque, addr + 0, (value) & 0xff, 8);
    omap3_scm_write8(opaque, addr + 1, (value >> 8) & 0xff, 8);
}
*/
                              ...
                              uint64_t value, unsigned size)
{
    ...
    omap3_scm_write8(opaque, addr + 0, (value) & 0xff,8);
    omap3_scm_write8(opaque, addr + 1, (value >> 8) & 0xff,8);
    omap3_scm_write8(opaque, addr + 2, (value >> 16) & 0xff,8);
    omap3_scm_write8(opaque, addr + 3, (value >> 24) & 0xff,8);
}

static const MemoryRegionOps omap3_scm_ops = {
    
    .read = omap3_scm_read32,
    .write = omap3_scm_write32,
    .endianness = DEVICE_NATIVE_ENDIAN,
};
...
struct omap3_sms_s
{
    ...
    uint32_t sms_sysconfig;
    uint32_t sms_sysstatus;
    uint32_t sms_rg_att[8];
    uint32_t sms_rg_rdperm[8];
    uint32_t sms_rg_wrperm[8];
    uint32_t sms_rg_start[7];
    uint32_t sms_rg_end[7];
    uint32_t sms_security_control;
    uint32_t sms_class_arbiter0;
    uint32_t sms_class_arbiter1;
    uint32_t sms_class_arbiter2;
    uint32_t sms_interclass_arbiter;
    uint32_t sms_class_rotation[3];
    uint32_t sms_err_addr;
    uint32_t sms_err_type;
    uint32_t sms_pow_ctrl;
    uint32_t sms_rot_control[12];
    uint32_t sms_rot_size[12];
    uint32_t sms_rot_physical_ba[12];
};
                                        ...
                                        Chardev *chr_uart4)
{
    ...
    s->cpu = ARM_CPU( cpu_create( ARM_CPU_TYPE_NAME("cortex-a8")) );
    ...
    s->l4 = omap_l4_init(sysmem, OMAP2_L4_BASE, L4A_COUNT);
