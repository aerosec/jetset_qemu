...
static void bcm2836_realize(DeviceState *dev, Error **errp)
{
    ...
    if (err) {
        ...
        //printf("cpus enabled, first_cpu = %p\n",ARM_CPU(first_cpu));
    ...
    //DeviceState *synth_dev = qdev_create(NULL, "synth_rpi");
    DeviceState *synth_dev;

    if (concrete_target == NULL)
    {
        synth_dev = qdev_create(NULL, "synth_rpi");
    }
    else if (strcmp(concrete_target,"rpi") == 0){
        synth_dev = qdev_create(NULL, "synth_rpi_concrete");
    }
    else {
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->peripherals), 0,
        qdev_get_gpio_in_named(DEVICE(synth_dev), BCM2835_IC_ARM_IRQ,
                               INTERRUPT_ARM_MAILBOX));

    /* bcm2836 interrupt controller (and mailboxes, etc.) */
    sysbus_connect_irq(SYS_BUS_DEVICE(synth_dev), 0,
        qdev_get_gpio_in_named(DEVICE(synth_dev), "gpu-irq", 0));
    ...
    for (n = 0; n < BCM283X_NCPUS; n++) {
        ...
        qdev_connect_gpio_out_named(DEVICE(synth_dev), "irq", n,
                qdev_get_gpio_in(DEVICE(&s->cpus[n]), ARM_CPU_IRQ));
       
        qdev_connect_gpio_out(DEVICE(&s->cpus[n]), GTIMER_VIRT,
                qdev_get_gpio_in_named(DEVICE(synth_dev), "cntvirq", n));
