...
static uint32_t x86_cpu_archname(CPUState *cs)
{
    return 3;
}
...
static uint64_t x86_cpu_get_pc(CPUState *cs)
{
    X86CPU *cpu = X86_CPU(cs);
    return cpu->env.segs[R_CS].base + cpu->env.eip;
}


static uint64_t x86_cpu_get_fp(CPUState *cs)
{
    X86CPU *cpu = X86_CPU(cs);
    return cpu->env.regs[R_EBP];
}

static uint64_t x86_cpu_get_reg(CPUState *cs, uint32_t regnum)
{
    X86CPU *cpu = X86_CPU(cs);
    return cpu->env.regs[regnum];
}

static uint64_t x86_cpu_read_stack(CPUState *cs, uint64_t addr, uint8_t* buf, uint64_t len)
{
    X86CPU *cpu = X86_CPU(cs);
    uint64_t stack_addr = cpu->env.segs[R_SS].base + addr;
    //printf("addr = 0x%lx stack_addr = 0x%lx\n", addr, stack_addr);
    return cpu_memory_rw_debug(cs, stack_addr, buf, len, false);
}
 

static uint64_t x86_cpu_maybe_adjust_addr(CPUState *cs, uint64_t addr)
{
    X86CPU *cpu = X86_CPU(cs);
    return cpu->env.segs[R_CS].base + addr;
}
...
static void x86_cpu_common_class_init(ObjectClass *oc, void *data)
{
    ...
    cc->archname = x86_cpu_archname;
    cc->get_pc = x86_cpu_get_pc;
    cc->get_reg = x86_cpu_get_reg;
    cc->get_fp = x86_cpu_get_fp;
    cc->read_stack = x86_cpu_read_stack;
    cc->maybe_adjust_addr =  x86_cpu_maybe_adjust_addr;
