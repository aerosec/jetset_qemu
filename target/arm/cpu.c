...
static uint32_t arm_cpu_archname(CPUState *cs)
{
    return 1;
}
...
static uint64_t arm_cpu_get_pc(CPUState *cs)
{
    ARMCPU *cpu = ARM_CPU(cs);
    return cpu->env.regs[15] | cpu->env.thumb;
}


static uint64_t arm_cpu_get_fp(CPUState *cs)
{
    ARMCPU *cpu = ARM_CPU(cs);

    return cpu->env.regs[11];
}


static uint64_t arm_cpu_get_reg(CPUState *cs, uint32_t regnum)
{
    ARMCPU *cpu = ARM_CPU(cs);

    return cpu->env.regs[regnum];
}


static uint64_t arm_cpu_read_stack(CPUState *cs, uint64_t addr, uint8_t* buf, uint64_t len)
{
    /* ARMCPU *cpu = ARM_CPU(cs); */
    //uint64_t stack_addr = cpu->segs[R_SS].base + addr
    return cpu_memory_rw_debug(cs, addr, buf, len, false);
}


static uint64_t arm_cpu_maybe_adjust_addr(CPUState *cs, uint64_t addr)
{
    ARMCPU *cpu = ARM_CPU(cs);
...
static void arm_cpu_reset(CPUState *s)
{
    ...
    if (arm_feature(env, ARM_FEATURE_M)) {
        ...
        //Allow override of intial pc
        //compensates for broken vector table read when necessary
        if (cortexm_initial_pc != -1)
            initial_pc = cortexm_initial_pc;
        if (cortexm_initial_sp != -1)
            initial_msp = cortexm_initial_sp;
...
static void arm_cpu_class_init(ObjectClass *oc, void *data)
{
    ...
    cc->get_fp = arm_cpu_get_fp;
    cc->archname = arm_cpu_archname;
    cc->read_stack = arm_cpu_read_stack;
    cc->maybe_adjust_addr =  arm_cpu_maybe_adjust_addr;
    cc->get_reg = arm_cpu_get_reg;
